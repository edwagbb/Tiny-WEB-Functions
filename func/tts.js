...
exports.default = async function (ctx) {
  ...
  if (!text) return 'text is require'
  ...
  async connect() {
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.on("message", (message, isBinary) => {
        let pattern =  /X-RequestId:([a-z|0-9]*)/;
        if (!isBinary) {
          let data = message.toString();
          if (data.includes("Path:turn.start")) {
            // 开始传输
            let matches = data.match(pattern);
            let requestId = matches ? matches[1] : null;
            ...
            if (executor) {
              this.executorMap.delete( matches ? matches[1] : null);
...
const retry = async function (fn, times, errorFn, failedMessage) {
  let reason = {
    message: failedMessage ? failedMessage:"多次尝试后失败",
    errors: [],
  };
